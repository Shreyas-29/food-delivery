...
import { removeFromCart, selectCartItems, selectCartTotal, selectTotalQuantity } from '../slices/cart';
...
const Cart = () => {
    ...
    return (
        <View className="relative flex-1 h-full py-8 bg-white">
            ...
            <View className="absolute inset-x-0 bottom-0 z-50 w-full bg-white">
                <View className="relative z-50 w-full rounded-3xl" style={{ backgroundColor: themeColors.bgColor(0.2), zIndex: 1000 }}>
                    <View className="z-50 p-6 px-8 space-y-3">
                        ...
                        <View className="flex-row items-center justify-between">
                            ...
                            <Text className="text-gray-700" style={{ fontFamily: "Medium" }}>
                                {cartTotal === 0 ? "$0.00" : `$${(cartTotal + deliveryFee).toFixed(2)}`}
                            </Text>
                        </View>
                        <TouchableOpacity
